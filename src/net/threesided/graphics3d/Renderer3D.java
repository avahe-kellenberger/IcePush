...
public class Renderer3D extends Renderer {
    ...
    private void doRender(Object3D[] objArray) {
        ...
        for (Object3D obj : objArray) {
            ...
            for (int currentFace = 0; currentFace < obj.faceVertices.length; currentFace++) {
                ...
                if (obj.faceVertices[currentFace] == null) continue;
                ...
                for (int currentVertex = 0; currentVertex < vertexCount; currentVertex++) {
                    ...
                    if (obj.vertZRelCam[vertexID] <= 0) obj.vertZRelCam[vertexID] = 1;

                    int[] screen =
                            worldToScreen(
                                    obj.vertXRelCam[vertexID],
        ...
        for (int k = faceIndex - 1; k >= 0; k--) {
            Face f = faceArray[k];
            if (f.color == -1) {
                perspectiveCorrectPolygon(
                        f.x,
                        f.y,
                        f.z,
                        f.u,
                        f.v,
                        Object3D.textures[f.texID],
                        Object3D.texsidelens[f.texID]);
            } else {
    ...
    public double[] transformPoint(
            double objBaseX, double objBaseY, double objBaseZ, double x, double y, double z) {
        x -= cameraX;
        y -= cameraY;
        z -= cameraZ;
    ...
    /**
     * Fills a textured triangle which is perspective-corrected at each pixel. Original
     * implementation at: http://www.lysator.liu.se/~mikaelk/doc/perspectivetexture/ Implemented in
     * Java by Tekk.
     */
    public void perspectiveCorrectTriangle(
            float x1,
            float y1,
            float z1,
            float x2,
        ...
        if (y2 > y1) dxdy1 = (x2 - x1) / (y2 - y1);
        if (y3 > y1) dxdy2 = (x3 - x1) / (y3 - y1);
        if (y3 > y2) dxdy3 = (x3 - x2) / (y3 - y2);
    ...
    /** A simple 'fan'-based method which triangulates and then textures any convex polygon. */
    public void perspectiveCorrectPolygon(
            int[] x, int[] y, float[] z, int[] u, int[] v, int[] texture, int sidelen) {
        for (int k = 2; k < x.length; k++) {
            perspectiveCorrectTriangle(
                    x[0], y[0], z[0], x[k - 1], y[k - 1], z[k - 1], x[k], y[k], z[k], u[0], v[0],
                    u[k - 1], v[k - 1], u[k], v[k], texture, sidelen);
        }
    }
}
