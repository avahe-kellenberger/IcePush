...
public class Object3D {
    ...
    public Object3D(
            double layerHeights[], double layerScale[], Color layerColors[], int baseSides) {
        double basePointX[] = new double[baseSides], basePointZ[] = new double[baseSides];

        for (int i = 0; i < baseSides; i++) {
            double ang = TWO_PI * (0.125F + ((double) i) / ((double) baseSides));
        ...
        if (numLayers != layerScale.length || numLayers != (1 + layerColors.length))
        ...
        for (int i = 0; i < numLayers; i++) {
                ...
                if (i != 0 && j != 0) {
                    ...
                    faceVertices[faceCount] =
                            new int[] {
                                vertexCount - 1,
                                vertexCount - 2,
                                vertexCount - (baseSides + 2),
                                vertexCount - (baseSides + 1)
                            };
                    faceColors[faceCount++] =
                            scaleColor(layerColors[i - 1], (5 - 2 * basePointX[j]) / 8);
                }
            }
            ...
            faceVertices[faceCount] =
                    new int[] {
                        vertexCount - 1,
                        vertexCount - baseSides,
                        vertexCount - (2 * baseSides),
                        vertexCount - (baseSides + 1)
                    };
            faceColors[faceCount++] = scaleColor(layerColors[i - 1], (5 - 2 * basePointX[0]) / 8);
        }
    ...
    static {
        ...
        Color treeLayerColors[] = new Color[] {new Color(128, 128, 0), Color.green, Color.green};

        templates = new Object3D[4];

        templates[0] = new Object3D(treeLayerHeights, treeLayerScale, treeLayerColors, 6);
        try {
        ...
        for (int k = 0; k < textures.length; k++) loadTexture(k, "models/" + k + ".jpg");
    }
