...
public class WebSocketBuffer extends PacketBuffer {
	...
	private String responseHeader = "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n";
		...
		if(headerEnd != -1) {		// This code only executes if the header ending was found  !!
			...
			for(String line : lines) {
				...
				if(parts.length == 2) {
					...
					String response = responseHeader + "Sec-WebSocket-Accept: " + generateKey(swsk) + "\r\n\r\n";
	...
	byte[] readOneFrame(byte[] data, int pos, int end) {
		...
		byte[] key = new byte[]{ data[pos + 2 + bonus], data[pos + 3 + bonus], data[pos + 4 + bonus], data[pos + 5 + bonus] };
		...
		//c.decoded = nbuf;
		//shutDown = true;
		//System.out.println("===== Read one Frame pos:" + pos + ", end:" + end + "  ===== nextPos = " + nextPos);
		return nbuf;
	}
	...
	byte[] encodeData(byte[] data, int len) {
		int packs = len/125;
		int tail = len%125;
		byte[] out = new byte[(packs)*(2 + 125) + (tail == 0? 0 : (2 + tail))];	// 125 is maximum size of a data frame without extended payload length bytes
	...
	public boolean synch() {
		...
		try {
			...
			if (available != 0) {
				...
				//System.out.println("newRaw length = " + newRaw.length + " rawInbuff.length = " + rawInBuff.length + " available: " + available);
			...
			//System.out.println("inBuf.len:" + inBuf.length + ", rawInBuff.len:" + rawInBuff.length + ", avail2:" + available2 + ", numBytesUnread:" + numBytesUnread + ", dataEnd:" + dataEnd + ", readPtr:" + readPtr);
	...
	byte[] rawInBuff;	// Contains any left over bytes from incoming websocket frames that are incomplete
	private int nextPos;
