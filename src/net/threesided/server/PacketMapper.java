...
public class PacketMapper {
	...
	// TODO: Load this from config or directory structure or search or all of the above or anything but this
	private static final String[] classes = new String[] {
		"net.threesided.server.Player"
	};

	/* c2s == client to server. s2c == server to client. We should evaluate/standardize usage of this convention at some point */

	private static final String[] c2sPacketNames = new String[] {
		"MOVE_REQUEST", "END_MOVE", "LOGOUT", "PING", "CHAT_REQUEST"
	};

	private static int[] opcodes = new int[] {
		MOVE_REQUEST, END_MOVE, LOGOUT, PING, CHAT_REQUEST
	};

	private static Hashtable<Integer, MethodHandle> methodTable = new Hashtable<Integer, MethodHandle>();
		...
		for(int i = 0; i < args.length; i++) {
			try {
				switch(handle.format[i]) {
					case STRING:
						if(b) { System.out.println("Reading string"); }
				...
				System.out.println("Malformed packet or something like that wrong:");		// THIS BLOCK SHOULD NOT EXIST
	...
	private static final String[] dataTypeNames = new String[] {
		"STRING", "BYTE", "SHORT", "INTEGER"
	};
