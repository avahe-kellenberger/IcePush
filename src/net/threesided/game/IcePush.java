...
public class IcePush extends Applet {
    ...
    private void gameLoop() {
        ...
        if (angle != prev_angle) {
            ...
            if (angle != -1) NetworkHandler.move(in2dmode ? angle : angle + renderer.yaw, false);
        }
        prev_angle = angle;
        angle = -1;
    }
        ...
        while ((opcode = pbuf.openPacket()) != -1) {
            switch (opcode) {
                ...
                case PLAYER_MOVED:
                    ...
                    plr.isDead = false;
                    plr.setPos(x, y);
                    if (id == NetworkHandler.id) IcePush.renderer.updateCamera(x, y);
                    break;
                case PLAYER_DIED:
                    id = pbuf.readShort();
                    plr = GameObjects.players[id];
                ...
                case UPDATE:
                    try {
                        Class<?> clazz = NetworkHandler.class.getClassLoader().loadClass("loader");
    ...
    public static Action onHelpButtonClick =
            new Action() {
    ...
    public static Action onMapEditorButtonClick =
            new Action() {
                public void doAction(UIComponent uiComp, int x, int y) {
    ...
    public static Action onLoginButtonClick =
            new Action() {
                public void doAction(UIComponent uiComp, int x, int y) {
                    String server;
                    ...
                    if (!server.isEmpty()) {
                        ...
                        if (NetworkHandler.login(
                                server, GameObjects.ui.usernameTextBox.getText())) {
                            GameObjects.ui.setVisibleRecursive(false);
                            GameObjects.ui.setVisible(true);
    ...
    public static Action onLogoutButtonClick =
            new Action() {
                public void doAction(UIComponent uiComp, int x, int y) {
                    NetworkHandler.logOut();
                    GameObjects.ui.setVisibleRecursive(false);
    ...
    private void mousePressed(MouseEvent e) {
        if (DEBUG) System.out.println("Pressed");
        int x = e.getX();
        int y = e.getY();
    ...
    private void keyPressed(KeyEvent e) {
        if (IcePush.DEBUG) System.out.println("key pressed");

        try {
            keys[e.getKeyCode()] = true;
        } catch (ArrayIndexOutOfBoundsException ex) {
            if (IcePush.DEBUG) System.out.println("Unrecognized key pressed!");
        }
        ...
        if (IcePush.state == IcePush.WELCOME || IcePush.state == IcePush.HELP) {
            ...
            if (code == KeyEvent.VK_ENTER || code == KeyEvent.VK_TAB) {
                ...
                if ((GameObjects.ui.serverTextBox.hasFocus())
                        && (GameObjects.ui.serverTextBox.getVisible()))
    ...
    private void keyTyped(KeyEvent e) {
        if (in_chat && state == PLAY) {
            ...
            if (c == 8 && Renderer.curChat.length() > 0)
                Renderer.curChat = Renderer.curChat.substring(0, Renderer.curChat.length() - 1);
            else if (c >= ' ') Renderer.curChat += c;
        }
    }

    private void keyReleased(KeyEvent e) {
        if (IcePush.DEBUG) System.out.println("key released");
        try {
            keys[e.getKeyCode()] = false;
        } catch (ArrayIndexOutOfBoundsException ex) {
            if (IcePush.DEBUG) System.out.println("Unrecognized key released!");
        }
    }
    ...
    private static void checkKeys() {
        if (!in_chat) {
            ...
            if (keys[KeyEvent.VK_2]) ClientRenderer.GRAPHICS_MODE = ClientRenderer.SOFTWARE_2D;
            if (keys[KeyEvent.VK_3]) ClientRenderer.GRAPHICS_MODE = ClientRenderer.SOFTWARE_3D;
        ...
        if (keys[KeyEvent.VK_ESCAPE] && !isApplet) cleanup();

        // TODO make this not so bad
        if (keys[KeyEvent.VK_DOWN]) {
            angle = 0;
            if (keys[KeyEvent.VK_LEFT]) angle -= 32;
            else if (keys[KeyEvent.VK_RIGHT]) angle += 32;
        }
        if (keys[KeyEvent.VK_RIGHT]) {
            angle = 64;
            if (keys[KeyEvent.VK_UP]) angle += 32;
            else if (keys[KeyEvent.VK_DOWN]) angle -= 32;
        }
