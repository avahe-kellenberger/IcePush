...
public class Triangles {
    ...
    public static void init(int w, int h, Component component) {
        ...
        for (int i = 0; i < height; i++) lineOffs[i] = i * width;
    }
    ...
    public static void setDimensions(int x, int y) {
        ...
        scaledWidth = x << 12;
        lineOffs = new int[y];
        for (int i = 0; i < y; i++) lineOffs[i] = i * width;
    }
    ...
    static void testMath(int x1, int y1, int x2, int y2, int x3, int y3, int z1, int z2, int z3) {
        ...
        float x = (y1 - y2) * (z1 - z3) - (y1 - y3) * (z1 - z2),
    ...
    public static void shadedTriangle(
            int x1, int y1, int x2, int y2, int x3, int y3, int color, int b1, int b2, int b3) {
        fillGradientTriangle(
                ...
                x3,
                y3,
                brightenColor(color, b1),
                brightenColor(color, b2),
                brightenColor(color, b3));
    }
    ...
    public static void clipHorz(
            int prevX, int prevY, int currX, int currY, int clipY, boolean toTop) {

        boolean prevIn = toTop ^ (prevY >= clipY); // Pixel is only above
        // boundary if strictly less
        boolean currIn = toTop ^ (currY >= clipY);

        if (toTop) clipY--; // So that calls down the line to emitVertex(.. clipY) are
        // actually in bounds
        ...
        if (prevIn && currIn) { // Both inside. Emit current.
            ...
            // then current.
            emitVertex(prevX + ((clipY - prevY) * (prevX - currX)) / (prevY - currY), clipY);
            emitVertex(currX, currY);
    ...
    public static void clipVert(
            int prevX, int prevY, int currX, int currY, int clipX, boolean toLeft) {
        ...
        if (toLeft) clipX--; // Ibid.

        if (prevIn && currIn) { // Both inside. Emit current.
            emitVertex(currX, currY);
        } else if ((!prevIn) && currIn) { // Prev out, curr in. Emit intserect
            // then current.
            emitVertex(clipX, prevY + ((clipX - prevX) * (prevY - currY)) / (prevX - currX));
            emitVertex(currX, currY);
        } else if (prevIn && (!currIn)) { // Curr out, prev in. Emit intersect.
            emitVertex(clipX, prevY + ((clipX - prevX) * (prevY - currY)) / (prevX - currX));
        }
    ...
    /**
     * Applies a gradient to a rectangle based on color given at three vertices. Used to test
     * gradient code for being simpler than triangles.
     */
    public static void testGradientRect(
            int x1, int y1, int x2, int y2, int x3, int y3, int color1, int color2, int color3) {
        int dy2_1 = y2 - y1;
        int dy3_1 = y3 - y1;
        ...
        float xred = (dy2_1) * (red3 - red1) - (dy3_1) * (red2 - red1),
                yred = (dx3_1) * (red2 - red1) - (dx2_1) * (red3 - red1);
        float xgreen = (dy2_1) * (green3 - green1) - (dy3_1) * (green2 - green1),
                ygreen = (dx3_1) * (green2 - green1) - (dx2_1) * (green3 - green1);
        float xblue = (dy2_1) * (blue3 - blue1) - (dy3_1) * (blue2 - blue1),
                yblue = (dx3_1) * (blue2 - blue1) - (dx2_1) * (blue3 - blue1);
        ...
        for (int i = minx; i < maxx; i++)
            for (int j = miny; j < maxy; j++)
                pixels[j * width + i] =
                        (((int) (i * xred + j * yred + zred)) << 16)
                                + (((int) (i * xgreen + j * ygreen + zgreen)) << 8)
                                + ((int) (i * xblue + j * yblue + zblue));

        pixels[width * y1 + x1] = pixels[width * y2 + x2] = pixels[width * y3 + x3] = 0xffffff;
    }

    /**
     * Generates a 64x64 texture gradiating from blue to green. Used to test perspective mapping.
     */
    static int[] genTexture() {
        ...
        for (int i = 0; i < 64; i++)
            for (int j = 0; j < 64; j++) {
                ...
                if (i == j) texture[i + j * 64] = 0xff; // Creates blue line of pixels
    ...
    /**
     * Implementation of the above. Maps a triangle. A rectangular viewport is used for testing
     * simplicity.
     */
    public static void texturemapRectTest(
            int texture[],
            int texwidth,
            int u1,
            int v1,
        ...
        if (!solve(x1 * z1, y1 * z1, u1, x2 * z2, y2 * z2, u2, x3 * z3, y3 * z3, u3))
            throw new RuntimeException("Singularity u");
        float ua = a, ub = b, uc = c;

        if (!solve(x1 * z1, y1 * z1, v1, x2 * z2, y2 * z2, v2, x3 * z3, y3 * z3, v3))
        ...
        if (!solve(x1 * z1, y1 * z1, z1, x2 * z2, y2 * z2, z2, x3 * z3, y3 * z3, z3))
        ...
        for (int i = minx; i <= maxx; i++) {
            for (int j = miny; j <= maxy; j++) {
                ...
                if (screenindex >= 0 && screenindex < pixels.length) pixels[screenindex] = pixel;
            }
        }
    ...
    /**
     * Fills a gradient triangle with colors color1, color2, color3 at vertices (x1, y1), (x2, y2),
     * (x3, y3) respectively. Except for a few clipping issues and the fact that it draws the left
     * edges one pixel short, this method works completely. This method should be retired in favor
     * of the new one that will work once generalized optimal sorting, clipping, linear solving, and
     * filling methods have been determined.
     */
    public static void fillGradientTriangle(
            int x1, int y1, int x2, int y2, int x3, int y3, int color1, int color2, int color3) {
        ...
        } else throw new InternalError("Sort routine failed. Check for severe hardware error.");
        ...
        if (det == 0) return; // Degenerate triangle; point/line
        ...
        if (dy2_1 != 0) {
            ...
            if (step2_1 > step3_1) { // first part: rightStep > leftStep
                ...
                for (int i = y1; i < y2; i++, left += step3_1, right += step2_1) {
                    ...
                    if (l < 0) l = 0;
                    if (r > boundx) r = boundx;
                    if (l >= r || i < 0 || i >= height) continue;

                    for (int j = l; j < r; j++) {

                        pixels[i * width + j] =
                                (((int) (j * xred + i * yred + zred)) << 16)
    ...
    public static void solidTriangle(int X1, int Y1, int X2, int Y2, int X3, int Y3, int color) {
        ...
        if (_y1 != _y2) {
            ...
            while (_y1 != _y2) {
                if (_y1 >= 0 && _y1 < height) {
                    ...
                    if (l < boundLeft) l = boundLeft;
                    if (r > boundRight) r = boundRight;

                    while (l < r) pixels[l++] = color;
                }
                left += stepLeft;
                right += stepRight;
                _y1++;
    ...
    /**
     * Sorts a trio of vertices by height so that y1 <= y2 <= y3 Implemented as its own method with
     * global variables so that triangle code can stay small (Crucial to fitting within CPU code
     * cache for these presumably highly performance intensive routines.)
     */
    ...
    /**
     * Old fillGradientTriangle routine. To my knowledge, this method works perfectly under all
     * conditions. Performance has been deemed suboptimal (Dismally slow), so research is now to be
     * directed into the routines described above.
     */
    public static void fgtOLD(
            int x1, int y1, int x2, int y2, int x3, int y3, int color1, int color2, int color3) {

        if ((y1 == y2 && y2 == 3) || (x1 == x2 && x2 == x3)) return;
            ...
            if (step2_1 > step3_1) { // first part: rightStep > leftStep
                ...
                for (int i = y1; i < y2; i++) {
                    ...
                    gradientScanline(
                            ...
                            right >> 12,
                            i,
                            gradePixel(color1, color3, i - y1, dy3_1),
                            gradePixel(color1, color2, i - y1, dy2_1));
            ...
            if (step3_2 < step3_1) {
                ...
                for (int i = y2; i < y3; i++) {
                    ...
                    gradientScanline(
                            ...
                            i,
                            gradePixel(color1, color3, i - y1, dy3_1),
                            gradePixel(color2, color3, i - y2, dy3_2));
    ...
    private static void gradientScanline(int start, int end, int y, int startC, int endC) {
        ...
        } else if (start > boundx) return;

        if (end < 0) return;
        else if (end > boundx) end = boundx;
     ...
     *
     * <p>a.x1 + b.y1 + c = z1 a.x2 + b.y2 + c = z2 a.x3 + b.y3 + c = z3.
     *
     * <p>Returns true if the equation was solvable, false otherwise.
     *
     * <p>This method uses integers, as it was determined float to integer & back conversion was
     * extremely slow. Using integers and left & right bit shifting for multiplication/division to
     * minimize errors was decided upon. Shift factor has not yet been determined. After it has
     * been, this method will be implementable. A shift factor of between 12 and 16 seems the most
     * desirable.
     */
    static boolean solve(int x1, int y1, int z1, int x2, int y2, int z2, int x3, int y3, int z3) {
        int dx12 = x1 - x2;
        int dx13 = x1 - x3;
    ...
    /**
     * This method works under all cases. Since the usage of floats has lead to suboptimal
     * performance, research is to be directed into the routine described above.
     */
    ...
    private static void gradientLine(
            int pixels[], int index, int start, int end, int startC, int endC) {
    ...
    /**
     * Draws a gradient triangle based on base & height. Was used mainly to generate the triangle
     * for http://jaghax.org/akrm/tsp.png then forgotten.
     */
    public static void gradientTriangle(
            int pixels[],
            int x,
            int y,
            int height,
        ...
        for (int i = 0; i < height; i++) {
            ...
            gradientLine(
                    pixels,
                    (i + y) * scanSize + x - scanWidth,
                    0,
                    2 * scanWidth,
                    gradePixel(topColor, leftColor, i, height),
    ...
    public static void fillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, int color) {
        ...
        if (maxLength == ydiff1) {
            ...
            for (; y3 < y4; y3++, factor1 += factorStep1, factor2 += factorStep2)
                drawHorizontalLine(
                        x1 + (int) (xdiff1 * factor1),
                        x3 + (int) (xdiff2 * factor2) + 1,
                        y3,
                        color);
            ...
            for (; y1 < y2; y1++, factor1 += factorStep1, factor2 += factorStep2)
                drawHorizontalLine(
                        x3 + (int) (xdiff2 * factor1),
                        x1 + (int) (xdiff1 * factor2) + 1,
                        y1,
                        color);
    ...
    public static void drawHorizontalLine(int fromX, int toX, int y, int color) {
        ...
        toX += y;
        while (fromX < toX) {
            if (i > 0 && i < width) pixels[fromX] = color;
            fromX++;
            i++;
        }
    }
        ...
        if (maxLength == ydiff1) {
            ...
            for (; y3 < y4; y3++, factor1 += factorStep1, factor2 += factorStep2)
                horizontalGradient(
                        ...
                        x3 + (int) (xdiff2 * factor2) + 1,
                        y3,
                        grade(vertColor2, vertColor1, i++, gradDist),
                        grade(vertColor2, vertColor3, y3, y4));
            factor1 = (y5 - y1) / (float) ydiff1;
            factorStep1 = 1.0f / ydiff1;
            factor2 = 0.0f;
            factorStep2 = 1.0f / ydiff3;
            for (j = 0; y5 < y6; y5++, factor1 += factorStep1, factor2 += factorStep2)
                horizontalGradient(
                        ...
                        y5,
                        grade(vertColor2, vertColor1, i++, gradDist),
                        grade(vertColor3, vertColor1, j++, y6));
        } else if (maxLength == ydiff2) {
            ...
            for (; y1 < y2; y1++, factor1 += factorStep1, factor2 += factorStep2)
                horizontalGradient(
                        ...
                        y1,
                        grade(vertColor2, vertColor1, i++, gradDist),
                        grade(vertColor3, vertColor1, y1, y2));
            factor1 = (y5 - y3) / (float) ydiff2;
            factorStep1 = 1.0f / ydiff2;
            factor2 = 0.0f;
            factorStep2 = 1.0f / ydiff3;
            ...
            for (j = 0; y5 < y6; y5++, factor1 += factorStep1, factor2 += factorStep2)
                horizontalGradient(
                        ...
                        y5,
                        grade(vertColor2, vertColor1, i++, gradDist),
                        grade(vertColor2, vertColor3, j++, y6));
        } else {
            ...
            for (; y1 < y2; y1++, factor1 += factorStep1, factor2 += factorStep2)
                horizontalGradient(
                        ...
                        x1 + (int) (xdiff1 * factor2) + 1,
                        y1,
                        grade(vertColor1, vertColor3, i++, gradDist),
                        grade(vertColor1, vertColor2, y1, y2));
            factor1 = (y3 - y5) / (float) ydiff3;
            factorStep1 = 1.0f / ydiff3;
            factor2 = 0.0f;
            factorStep2 = 1.0f / ydiff2;
            for (j = 0; y3 < y4; y3++, factor1 += factorStep1, factor2 += factorStep2)
                horizontalGradient(
                        ...
                        y3,
                        grade(vertColor1, vertColor3, i++, gradDist),
                        grade(vertColor2, vertColor3, j++, y4));
        }
    }

    public static void horizontalGradient(int fromX, int toX, int y, int fc, int tc) {
        if (y < 0 || y > height) return;
        int i = 0;
        if (fromX > toX) {
            i = toX;
        ...
        toX += y;
        while (fromX < toX) {
            if (i > 0 && i < width) pixels[fromX] = grade(fc, tc, i, xdiff);
            fromX++;
            i++;
        }
    }

    private static int grade(int fromColor, int toColor, float numer, float denom) {
