...
public class InterthreadQueue<T> {
    ...
    public T pull() {
        Node<T> tt =
                tail.prev; // Local reference created to ensure that the value does not change
                           // concurrently after being read.
        if (tt == null) return null; // If this is non-null, we know that tail != head
        tail = tt;
        return tt.item;
    }
